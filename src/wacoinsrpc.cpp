...
// Copyright (c) 2017 WACoins developers
...
void ScriptPubKeyToJSON(const CScript& scriptPubKey, Object& out)
{
    ...
    BOOST_FOREACH(const CTxDestination& addr, addresses)
        a.push_back(CWACoinsAddress(addr).ToString());
...
void TxToJSON(const CTransaction& tx, Object& txdata)
{
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        ...
        if (ExtractDestinations(txout.scriptPubKey, txtype, addresses, nRequired))
        {
            ...
            Array addrs;
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                addrs.push_back(CWACoinsAddress(addr).ToString());
...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wacoins server stopping";
}
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    GenerateWACoinss(fGenerate, pwalletMain);
    return Value::null;
}
...
// wacoins: get network Gh/s estimate
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new wacoins address for receiving payments.  "
    ...
    return CWACoinsAddress(keyID).ToString();
}


CWACoinsAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CWACoinsAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current wacoins address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <wacoinsaddress> <account>\n"
            "Sets the account associated with the given address.");

    CWACoinsAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid wacoins address");
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CWACoinsAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CWACoinsAddress& address = item.first;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "sendtoaddress <wacoinsaddress> <amount> [comment] [comment-to]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <wacoinsaddress> <message>\n"
    ...
    CWACoinsAddress addr(strAddress);
    if (!addr.IsValid())
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <wacoinsaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <wacoinsaddress> [minconf=1]\n"
            "Returns the total amount received by <wacoinsaddress> in transactions with at least [minconf] confirmations.");

    // WACoins address
    CWACoinsAddress address = CWACoinsAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid WACoins address");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
            "sendfrom <fromaccount> <towacoinsaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
            "requires wallet passphrase to be set with walletpassphrase first");
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
    ...
    string strAccount = AccountFromValue(params[0]);
    CWACoinsAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid wacoins address");
    int64 nAmount = AmountFromValue(params[2]);
    if (nAmount < MIN_TXOUT_AMOUNT)
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    set<CWACoinsAddress> setAddress;
    vector<pair<CScript, int64> > vecSend;

    int64 totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CWACoinsAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid wacoins address:")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a wacoins address or hex-encoded public key\n"
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        // Case 1: wacoins address and we have full public key:
        CWACoinsAddress address(ks);
        if (address.IsValid())
        {
            CKeyID keyID;
    ...
    return CWACoinsAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CWACoinsAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CWACoinsAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CWACoinsAddress, tallyitem>::iterator it = mapTally.find(address);
...
void ListTransactions(const CWalletTx& wtx, const string& strAccount, int nMinDepth, bool fLong, Array& ret)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& s, listSent)
        {
            ...
            entry.push_back(Pair("address", CWACoinsAddress(s.first).ToString()));
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& r, listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                entry.push_back(Pair("address", CWACoinsAddress(r.first).ToString()));
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // wacoins: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; wacoins server stopping, restart to run with encrypted wallet";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <wacoinsaddress>\n"
            "Return information about <wacoinsaddress>.");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "wacoins is not connected!"); //

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "wacoins is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.wacoins.it/wiki/BIP_0022 for full specification.");
...
// wacoins: get information of sync-checkpoint
...
// wacoins: reserve balance from being staked for network protection
...
// wacoins: check wallet integrity
...
// wacoins: make a public-private key pair
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        {
            CWACoinsAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(-5, string("Invalid WACoins address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        {
            entry.push_back(Pair("address", CWACoinsAddress(address).ToString()));
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: wacoins-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
//
// JSON-RPC protocol.  WACoins speaks version 1.0 for maximum compatibility,
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use wacoinsd";
