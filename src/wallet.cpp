...
// wacoins: optional setting to unlock wallet for block minting only;
...
// wacoins: total coins staked (non-spendable until maturity)
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (int i = 0; i < pcoin->vout.size(); i++)
            {
                if (pcoin->nTime > nSpendTime)
                    continue;  // wacoins: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // wacoins: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // wacoins: not avatar mode
                    {
                        ...
                        // change transaction isn't always pay-to-wacoins-address
...
// wacoins: create coin stake transaction
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CWACoinsAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CWACoinsAddress(address).ToString());
}
...
// wacoins: check 'spent' consistency between wallet and txindex
// wacoins: fix wallet spent state according to txindex
...
// wacoins: disable transaction (only for coinstake)
