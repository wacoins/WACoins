...
const string strMessageMagic = "WACoins Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // wacoins: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // wacoins: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // wacoins: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// wacoins: find block wanted by given orphan block
...
int64 GetProofOfWorkReward(int nHeight, unsigned int nBits)
{
    ...
    // wacoins: subsidy is cut in half every 16x multiply of difficulty
    ...
    // Premine check
    if (nHeight == nBlockToPremine)
    {
        // 30% premine
        nSubsidy = (MAX_MONEY * 30 / 100);
        
        return nSubsidy;
    }
    // First 1024 blocks are to set up the network
    // and avoid bad behaviours
    else if (nHeight < 1024)
    {
        nSubsidy = (nSubsidy / 100) * 60;
    }
    else
    {
...
// wacoins: miner's coin stake is rewarded based on coin age spent (coin-days)
int64 GetProofOfStakeReward(int64 nCoinAge)
{
    static int64 nRewardCoinYear = 1 * CENT;  // creation amount per coin-year
    // (block reward x 33) x 365 = Total # new coins mined via PoW in 1 year.
    int64 nSubsidy = nCoinAge * 33 / (365 * 33 + 2) * nRewardCoinYear;
    if (fDebug && GetBoolArg("-printcreation"))
...
// wacoins: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // wacoins: target change every block
    // wacoins: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // wacoins: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal wacoins miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // wacoins: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // wacoins: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // wacoins: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // wacoins: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // wacoins: track money supply and mint amount info
    ...
    // wacoins: fees are not collected by miners as in wacoins
    // wacoins: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// wacoins: total coin age spent in transaction, in the unit of coin-days.
...
// wacoins: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // wacoins: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // wacoins: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // wacoins: record proof-of-stake hash value
    ...
    // wacoins: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // wacoins: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // wacoins: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock()
{
    ...
    // wacoins: check that the block satisfies synchronized checkpoint
    ...
    // wacoins: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // wacoins: verify hash target and signature of coinstake tx
    ...
    // wacoins: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // wacoins: getblocks may not obtain the ancestor block rejected
    ...
    // wacoins: if responsible for sync-checkpoint send it
...
// wacoins: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "WACoins", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "WACoins", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "NYTimes - 2017, Jul 17 - Behind Bucks County Killings, a Young Life Skidding Off the Rails";
        CTransaction txNew;
        txNew.nTime = 1500287307;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x5e0dc4efc2cca9004b545eb22afaf157b6ef2fac2a429d7b938e6f0748dd14e7"));
        ...
        */
        ...
        // wacoins: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // wacoins: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // wacoins: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // wacoins: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // wacoins: should not enter safe mode for longer invalid chain
    // wacoins: if sync-checkpoint is too old do not enter safe mode
    ...
    // wacoins: if detected invalid checkpoint enter safe mode
    ...
    // wacoins: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // wacoins: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // wacoins: record my external IP reported by peer
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // wacoins: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // wacoins: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // wacoins: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // wacoins: simplify transaction fee - allow free = false
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("WACoinsMiner : proof-of-work not meeting target");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WACoinsMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("WACoinsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadWACoinsMiner(void* parg);

static bool fGenerateWACoinss = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void WACoinsMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateWACoinss || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateWACoinss) && !fProofOfStake)
                return;
        }
        ...
        if (fProofOfStake)
        {
            // wacoins: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
        ...
        printf("Running WACoinsMiner with %d transactions in block\n", pblock->vtx.size());
...
void static ThreadWACoinsMiner(void* parg)
{
    ...
    try
    {
        ...
        WACoinsMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadWACoinsMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadWACoinsMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadWACoinsMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateWACoinss(bool fGenerate, CWallet* pwallet)
{
    fGenerateWACoinss = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateWACoinss = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d WACoinsMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!CreateThread(ThreadWACoinsMiner, pwallet))
                printf("Error: CreateThread(ThreadWACoinsMiner) failed\n");
            Sleep(10);
        }
    }
}
