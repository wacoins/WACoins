...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx)
{
...

    {
                    ...
                    if (wallet->IsMine(txout))
                    {
                        ...
                        if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address))
                        {
                            if (wallet->mapAddressBook.count(address))
                            {
                                ...
                                strHTML += tr("<b>To:</b> ");
                                strHTML += GUIUtil::HtmlEscape(CWACoinsAddress(address).ToString());
        ...
        if (!wtx.mapValue["to"].empty())
        {
            ...
            strHTML += tr("<b>To:</b> ");
            CTxDestination dest = CWACoinsAddress(strAddress).Get();
        ...
        if (wtx.IsCoinBase() && nCredit == 0)
        {
            ...
            if (wtx.IsInMainChain())
                strHTML += tr("(%1 matures in %2 more blocks)")
                        .arg(WACoinsUnits::formatWithUnit(WACoinsUnits::BTC, nUnmatured))
        ...
        else if (nNet > 0)
        {
            ...
            // Credit
            //
            strHTML += tr("<b>Credit:</b> ") + WACoinsUnits::formatWithUnit(WACoinsUnits::BTC, nNet) + "<br>";
        }
        else
        {
            ...
            if (fAllFromMe)
            {
                ...
                BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                {
                    ...
                    strHTML += tr("<b>Debit:</b> ") + WACoinsUnits::formatWithUnit(WACoinsUnits::BTC, -txout.nValue) + "<br>";
                }
                ...
                if (fAllToMe)
                {
                    ...
                    strHTML += tr("<b>Debit:</b> ") + WACoinsUnits::formatWithUnit(WACoinsUnits::BTC, -nValue) + "<br>";
                    strHTML += tr("<b>Credit:</b> ") + WACoinsUnits::formatWithUnit(WACoinsUnits::BTC, nValue) + "<br>";
                }

                int64 nTxFee = nDebit - wtx.GetValueOut();
                if (nTxFee > 0)
                    strHTML += tr("<b>Transaction fee:</b> ") + WACoinsUnits::formatWithUnit(WACoinsUnits::BTC,-nTxFee) + "<br>";
            }
            else
            {
                //
                ...
                BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                    if (wallet->IsMine(txin))
                        strHTML += tr("<b>Debit:</b> ") + WACoinsUnits::formatWithUnit(WACoinsUnits::BTC,-wallet->GetDebit(txin)) + "<br>";
                BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                    if (wallet->IsMine(txout))
                        strHTML += tr("<b>Credit:</b> ") + WACoinsUnits::formatWithUnit(WACoinsUnits::BTC,wallet->GetCredit(txout)) + "<br>";
            }
        }

        strHTML += tr("<b>Net amount:</b> ") + WACoinsUnits::formatWithUnit(WACoinsUnits::BTC,nNet, true) + "<br>";
        ...
        if (fDebug)
        {
            ...
            BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                if(wallet->IsMine(txin))
                    strHTML += "<b>Debit:</b> " + WACoinsUnits::formatWithUnit(WACoinsUnits::BTC,-wallet->GetDebit(txin)) + "<br>";
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                if(wallet->IsMine(txout))
                    strHTML += "<b>Credit:</b> " + WACoinsUnits::formatWithUnit(WACoinsUnits::BTC,wallet->GetCredit(txout)) + "<br>";
                    ...
                    if(txdb.ReadDiskTx(prevout.hash, prev))
                    {
                        if (prevout.n < prev.vout.size())
                        {
                            ...
                            if (ExtractDestination(vout.scriptPubKey, address))
                            {
                                if (wallet->mapAddressBook.count(address) && !wallet->mapAddressBook[address].empty())
                                    strHTML += GUIUtil::HtmlEscape(wallet->mapAddressBook[address]) + " ";
                                strHTML += QString::fromStdString(CWACoinsAddress(address).ToString());
                            }
                            strHTML = strHTML + " Amount=" + WACoinsUnits::formatWithUnit(WACoinsUnits::BTC,vout.nValue);
