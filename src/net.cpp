...
// We now get our external IP from a service show my ip first and only use this as a backup
...
{
    // Fallback in case service to show ip fails to get it
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "WACoins " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
// testnet dns seed begins with 't', all else are wacoins dns seeds.
static const char *strDNSSeed[][2] = {
    {"seed", "dns.wacoins.net"},
    {"server-2", "209.126.111.161"},
    {"server-3", "145.239.124.229"},
    {"server-1", "91.121.164.113"},
    {"tseed", "tdns.wacoins.net"},
};
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (true /*!fTestNet*/)  // wacoins enables dns seeding with testnet too
    {
...
unsigned int pnSeed[] =
{
    0x87ce09b0, 0xa16f7ed1, 0xe57cef91, 0x71a4795b,
};
...
void ThreadOpenConnections2(void* parg)
{
    ...
    loop
    {
        ...
        // Add seed nodes if service show ip isn't working
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  WACoins is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    GenerateWACoinss(GetBoolArg("-gen", false), pwalletMain);

    // wacoins: mint proof-of-stake blocks in the background
...
bool StopNode()
{
    ...
    if (vnThreadsRunning[THREAD_MINER] > 0) printf("ThreadWACoinsMiner still running\n");
